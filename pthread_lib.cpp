...
const int sz = 10000; //Maximum possible number of vertices. Preallocating space for DataStructures accordingly //Should always have 1 more than the specified number of nodes in the dijkstra_custom.cpp
...
void *work(void *threadarg) {
    ...
    int i, tid, rc, source, firstNeighbor, curNeighbor, combinedWeight, * cw, * nodes_per_thread, * dis;
    tid = my_data->tid;
...
int * Dijkstra(int source, int n, vector<pair<int,int> > adj[], int dis[]) //Algorithm for SSSP
{
    ...
    for (t = 0; t < NUM_THREADS; t++) {
        thread_data_array[t] = {t, n, source, adj, &curNeighbors, &pq, &nodes_per_thread, &cw, vis, dis, &solved};
