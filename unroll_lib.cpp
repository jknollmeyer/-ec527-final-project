...
#if defined(TwoUnroll) 
#define loopUnroll 2
#elif  defined(FourUnroll)
#define loopUnroll 4
#elif defined(EightUnroll)
#define loopUnroll 8
#elif defined(TenUnroll)
...
int * Dijkstra(int source, int n, vector<pair<int,int> > a[],int dis[]) //Algorithm for SSSP
{
    ...
    while(!pq.empty())
    {
        ...
        vis[cv]=true;
        int i = 0;
        for(i=0;i<a[cv].size() - loopUnroll;i += loopUnroll){ //Iterating through all adjacent vertices
